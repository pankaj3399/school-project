    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f9f9f9; padding: 30px; border-radius: 8px; margin-bottom: 30px;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${url}?${(role === Role.Student && !isStudent) ? `token=${encodeURIComponent(otp)}` : `otp=${encodeURIComponent(otp)}`}&role=${encodeURIComponent(role)}&email=${encodeURIComponent(email)}&isStudent=${encodeURIComponent(String(isStudent))}&toVerify=${encodeURIComponent(toVerify ?? "")}"
