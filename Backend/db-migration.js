...
const SOURCE_DB_URL = 'sourceDbUrl';
const DESTINATION_DB_URL = 'destinationDbUrl';
...
class DatabaseMigrator {
    ...
    async connect() {
        try {
            ...
            this.sourceConnection = await mongoose.createConnection(SOURCE_DB_URL).asPromise();
            console.log('✅ Connected to source database');

            this.destinationConnection = await mongoose.createConnection(DESTINATION_DB_URL).asPromise();
    ...
    async migrateCollection(collectionName) {
        try {
            ...
            const destinationCollection = this.destinationConnection.db.collection(collectionName);
                ...
                try {
                    if (SKIP_EXISTING) {
                        const existingIds = await destinationCollection
                            .find({ _id: { $in: batch.map(doc => doc._id) } }, { projection: { _id: 1 } })
                            .toArray();

                        const existingIdSet = new Set(existingIds.map(doc => doc._id.toString()));
                        const newDocuments = batch.filter(doc => !existingIdSet.has(doc._id.toString()));

                        if (newDocuments.length > 0) {
                            await destinationCollection.insertMany(newDocuments, { ordered: false });
                    ...
                    if (LOG_PROGRESS && (migratedCount + skippedCount) % 1000 === 0) {
                        console.log(`📈 Progress: ${migratedCount + skippedCount}/${totalCount} documents`);
                    }
            ...
            console.log(`✅ ${collectionName} completed: ${migratedCount} migrated, ${skippedCount} skipped`);
    ...
    async migrateIndexes(collectionName) {
        try {
            ...
            console.log(`🗂️  Migrating ${customIndexes.length} indexes for ${collectionName}...`);
            ...
            console.error(`❌ Index migration error for ${collectionName}:`, error.message);
        }
    }
    ...
    printSummary() {
        ...
        console.log(`Collections: ${this.stats.migratedCollections}/${this.stats.totalCollections}`);
        console.log(`Documents: ${this.stats.migratedDocuments}/${this.stats.totalDocuments}`);
        ...
        if (this.stats.errors.length > 0) {
            ...
            this.stats.errors.forEach((error, index) => {
                console.log(`${index + 1}. Collection: ${error.collection} - ${error.error}`);
            });
        }
...
async function runMigration() {
    ...
    if (DESTINATION_DB_URL === 'mongodb://localhost:27017/destination_database_name') {
