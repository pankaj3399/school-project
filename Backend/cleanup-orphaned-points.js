...
const DRY_RUN = false; // Change to false to actually delete data
...
const waitForConfirmation = () => {
  return new Promise((resolve) => {
    ...
    rl.question(
      '⚠️  Are you sure you want to proceed with deleting orphaned points history? Type "YES" to confirm: ',
      (answer) => {
        rl.close();
        resolve(answer === "YES");
      },
...
async function cleanupOrphanedPointsHistory() {
  ...
  try {
    ...
    log(
      `🔧 Configuration: DRY_RUN=${DRY_RUN}, REQUIRE_CONFIRMATION=${REQUIRE_CONFIRMATION}`,
    );
    ...
    const pointsHistoryStudentIds =
      await PointsHistory.distinct("submittedForId");
    log(
      `Found ${pointsHistoryStudentIds.length} unique student IDs in points history`,
    );
    ...
    for (const studentId of pointsHistoryStudentIds) {
      if (!existingStudentIds.has(studentId.toString())) {
        ...
        const orphanedRecords = await PointsHistory.find({
          submittedForId: studentId,
        }).lean();
        const totalPoints = orphanedRecords.reduce(
          ...
          dateRange: {
            earliest:
              orphanedRecords.length > 0
                ? new Date(
                    Math.min(
                      ...orphanedRecords.map((r) => new Date(r.submittedAt)),
                    ),
                  )
                : null,
            latest:
    ...
    log(
      `   • Student IDs in points history: ${pointsHistoryStudentIds.length}`,
    );
    ...
    logWarning(
      `Found ${orphanedStudentIds.length} orphaned student IDs with points history:`,
    );
    ...
    for (const info of orphanedRecordsInfo) {
      ...
      log(
        `      - Date Range: ${info.dateRange.earliest?.toISOString()} to ${info.dateRange.latest?.toISOString()}`,
      );
    }
    ...
    if (DRY_RUN) {
      logWarning(
        "DRY RUN: No data was actually deleted. To perform the cleanup:",
      );
      logWarning("1. Set DRY_RUN = false in the script");
      logWarning(
        "2. Optionally set REQUIRE_CONFIRMATION = false to skip confirmation",
      );
    ...
    for (const studentId of orphanedStudentIds) {
      try {
        ...
        logSuccess(
          `Deleted ${deleteResult.deletedCount} records for student ${studentId}`,
        );
      } catch (error) {
    ...
    if (remainingOrphanedRecords === 0) {
      logSuccess(
        `🎉 Cleanup completed successfully! Deleted ${totalDeleted} orphaned points history records.`,
      );
    } else {
      logWarning(
        `⚠️  Cleanup completed but ${remainingOrphanedRecords} orphaned records still remain. Manual review may be needed.`,
      );
    }
  } catch (error) {
