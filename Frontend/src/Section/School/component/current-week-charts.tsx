...
import { getHistoryOfCurrentWeek } from "@/api";
import { useNavigate } from 'react-router-dom';

const CurrentWeekCharts = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [awardRes, deductRes, withdrawRes] = await Promise.all([
    ...
    const PointsBarChart = ({ 
        ...
        <div className="w-full grid grid-cols-8 place-items-center border-2 py-5 rounded-lg h-64 mb-6 mt-10">
            ...
            <ResponsiveContainer className={"col-span-6"} width="100%" height="100%">
                <BarChart
                ...
                >
                    ...
                    <Tooltip />
                    <Bar background={{ fill: '#f5f5f5' }} dataKey="points" fill={barColor} />
                </BarChart>
            </ResponsiveContainer>
            ...
            onClick={() => {
                switch(title) {
                    case 'Tokens': navigateToDetails('AwardPoints'); break;
                    case 'Withdrawals': navigateToDetails('PointWithdraw'); break;
                    case 'Oopsies': navigateToDetails('DeductPoints'); break;
                }
            }}
            className='p-2 rounded-lg text-xs' style={{
